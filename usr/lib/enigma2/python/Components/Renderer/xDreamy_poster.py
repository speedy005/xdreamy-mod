# -*- coding: utf-8 -*-
# by digiteng...07.2021, ist die PosterX geht in Py2 und Py3
...
# so muss es sein
#<widget source="session.Event_Now" render="PosterX" path="/tmp/poster/" nexts="2" position="1040,99" size="200,300" zPosition="9" transparent="1" borderWidth="2" #     borderColor="1-Sender" />
#<widget source="session.Event_Next" render="PosterX" path="/tmp/poster/" nexts="2" position="1040,521" size="200,300" zPosition="9" transparent="1"
#     borderWidth="2" borderColor="1-Sendungnext" />
		...
		try:
			for i in range(self.nxts):
				...
				if srch != "movie":
					...
					if srch != "tv":
						srch = "multi"
				open("/tmp/poster1_sendung.txt", "w").write(evntNm)
				evntNm = evntNm.replace('\xc2\x86', '').replace('\xc2\x87', '')
				open("/tmp/poster1_sendungreplace.txt", "w").write(evntNm)
				pstrNm = self.pth + evntNm + ".jpg"
